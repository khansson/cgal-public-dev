...
#ifndef CGAL_SHAPE_DETECTION_RANSAC_ON_POINTS_3_TORUS_H
#define CGAL_SHAPE_DETECTION_RANSAC_ON_POINTS_3_TORUS_H
...
namespace CGAL {
	...
	/*!
		...
		\tparam Traits a model of `EfficientRANSACTraits` with the additional requirement for tori (see `EfficientRANSACTraits` documentation).
	*/
	...
	protected:
		...
		virtual void cos_to_normal(const std::vector<std::size_t> &indices, 
			...
			for (std::size_t i = 0;i<indices.size();i++) {
				...
				Vector_3 in_plane = this->cross_pdct(m_axis, this->cross_pdct(m_axis, d));
				if (this->scalar_pdct(in_plane, d) < 0)
					in_plane = this->scale(in_plane, FT(-1.0));
...
#endif // CGAL_SHAPE_DETECTION_RANSAC_ON_POINTS_3_TORUS_H
