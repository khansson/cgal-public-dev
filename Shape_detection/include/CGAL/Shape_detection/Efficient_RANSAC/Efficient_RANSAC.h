...
#ifndef CGAL_SHAPE_DETECTION_RANSAC_ON_POINTS_3_EFFICIENT_RANSAC_H
#define CGAL_SHAPE_DETECTION_RANSAC_ON_POINTS_3_EFFICIENT_RANSAC_H
	...
	/// @{
		...
		*/ 
								...
								= cpp11::function<bool(double)>())
		{
			...
			std::size_t limit_failed_candidates = (std::max)(std::size_t(10000), std::size_t(m_input_iterator_beyond - m_input_iterator_first) / std::size_t(100));
			...
			do { // main loop
				...
				best_candidate->m_score = 
					m_global_octree->score(best_candidate, m_shape_index, FT(3) * m_options.epsilon, m_options.normal_threshold);
					...
					m_options.min_points) 
				{
					...
					if (stop_probability((std::size_t) best_candidate->expected_value(),
						...
						// 2. remove the points
						const std::vector<std::size_t> &indices_points_best_candidate = best_candidate->indices_of_assigned_points();

						// update generated candidates to reflect removal of points
						generated_candidates = std::size_t(std::pow (1.f - (indices_points_best_candidate.size() / float(m_num_available_points - num_invalid)), 3.f) * generated_candidates);
...
#endif // CGAL_SHAPE_DETECTION_RANSAC_ON_POINTS_3_EFFICIENT_RANSAC_H
